...
Context::
Context(Memory&  mem, const FileSet*  fset):
memory(mem),
pc(0),
bp((mem.get_size()-(word_size*2)+(word_size-1))/word_size*word_size),
state(0)
{
  sp = bp;
...
/*
  auto  n = args.size();
    ...
    for(int  i = 0;  i < n;  i += 1)
    {
      auto&  m = st.member_list[i];
      auto&  l = args[i];

        switch(m.type.get_kind())
        {
      case(TypeKind::int8  ): mem.put8( bp+m.offset,l->i);break;
      case(TypeKind::uint8 ): mem.put8( bp+m.offset,l->i);break;
      case(TypeKind::int16 ): mem.put16(bp+m.offset,l->i);break;
      case(TypeKind::uint16): mem.put16(bp+m.offset,l->i);break;
      case(TypeKind::int32 ): mem.put32(bp+m.offset,l->i);break;
